...
- [x] Extend packet tooling to classify protocol 03 object/state packet families, not only protocol headers.
  - Current generated report parses 8,476 protocol 03 object messages, fully segments 6,770 messages, and identifies 14,486 known selector segments across movement, object state, spawn/profile, emote/effect, compact terminal selector `28` markers, selector `80` self-view attributes, selector `09` two-byte-prefixed movement positions, primary/secondary/adjacent movement wrappers, selector `9b` state blocks, delete-view, and static object/door families.
- [x] Ingest compact hex-only packet lines from community dumps.
  - Current generated report includes compact packet lines from movement and door captures, increasing packet-like lines by 27 and surfacing full compact door-answer payloads that were previously skipped by the spaced-byte parser.
...
- [x] Extract protocol 03 static object/door spawn prefix leads.
  - Current generated report finds nine selector `9e`/`a0` static object leads with a stable `cd ab` separator after compact door-answer lines are included. Selector `a0` carries static object id `888143880` across seven records, protocol object type `3`, six instance bytes, two matching static rows, one vendor inventory row, quaternion sample `0,-0.707,0,0.707`, and one full 60-byte payload whose post-quaternion body starts marker `41`, vector `-68200,145,17100`, nearest static-row distance `1272.8`, and tail `34 08 00 00 04 00 00 00 00`. Selector `9e` carries static object id `888143885` in two records, protocol object type `3`, instance bytes `93` and `ea`, two matching static rows, no vendor inventory row, quaternion sample `0,0.707,0,-0.707`, and full post-quaternion body samples with marker `41`, vector `-67400,145,13100`, nearest static-row distance `424.3`, and short tail variants.
