...
- [x] Group protocol 03 movement-state post-position tail lead families.
  - Current generated report keeps update-count-3 movement-state tails after first selectors `08` and `09` unresolved when variable tail bytes collide with generic selectors. It groups 298 tails into lead families, led by first-selector `08` with `ff 01 64 01` (181 records), first-selector `08` with `ff 01 4e 01` (67), first-selector `08` with `80 80` (23), first-selector `09` with `ff 01 64 01` (20), first-selector `09` with `80 80` (4), and first-selector `09` with `ff 01 4e 01` (3). The tail selector byte plus the next three payload bytes now decode as a little-endian tag; adjacent captures commonly step by 124-126 or 250-251, making it a timestamp/sequence lead.
...
- [x] Extract protocol 03 static object/door spawn prefix leads.
  - Current generated report finds 14 selector `9e`/`a0` static object leads with a stable `cd ab` separator after compact, label-prefixed compact, and loose manual door-answer blocks are included. Selector `a0` carries static object id `888143880` across nine full 60-byte records, protocol object type `3`, eight instance bytes, two matching static rows, one vendor inventory row, quaternion sample `0,-0.707,0,0.707`, marker `41`, vector `-68200,145,17100`, nearest static-row distance `1272.8`, and tail variants including `34 08 00 00 03/04/05/0e/12 00 00 00 00`. Selector `9e` carries static object id `888143885` in five full records, protocol object type `3`, five instance bytes, two matching static rows, no vendor inventory row, quaternion sample `0,0.707,0,-0.707`, marker `41`, vector `-67400,145,13100`, nearest static-row distance `424.3`, and tail variants `34 08 00 00 08/10/12 00 00 00 00`.
